...
export function memoryReport(title, runs, conversation, workspace) {
  ...
  return [
    workspace?.workflow?.blocked_events ? '# День 15 · Контролируемый жизненный цикл' : workspace?.invariants ? '# День 14 · Инварианты задачи' : workspace?.workflow ? '# День 13 · Состояние задачи' : personalized ? '# День 12 · Персонализация и память' : '# День 11 · Три слоя памяти', '', title || 'Алгоритмическая задача', '',
    ...(workspace?.workflow?.blocked_events ? ['## Подтверждения версий', '',
      `Утверждён план v${workspace.workflow.state.approved_plan_revision ?? '—'}, условие v${workspace.workflow.state.approved_task_revision ?? '—'}, правила v${workspace.workflow.state.approved_invariant_revision ?? '—'}.`,
      `На проверке решение v${workspace.workflow.state.submitted_solution_revision ?? '—'}. Принятый отчёт: ${workspace.workflow.state.accepted_validation_id ?? 'нет'}; проверенное решение v${workspace.workflow.state.validated_solution_revision ?? '—'}.`, '',
      'Переходы и точные ID версий контролирует backend. Принятие отчёта пользователем не означает выполнение тестов приложением. Изменение технического контекста требует нового плана. Системное правило этапа проверяется judge даже без пользовательских инвариантов.', ''] : []),
    ...
    ...(workspace?.workflow ? ['## Жизненный цикл', '', `Фаза: ${workspace.workflow.state.phase}. Статус: ${workspace.workflow.state.status}. Шаг: ${workspace.workflow.state.current_step_id || 'не выбран'}. Ожидается: ${workspace.workflow.state.expected_action}. Версия: ${workspace.workflow.state.revision}.`, '',
      'Переходы выполняет backend по явным командам. Артефакты и журнал приведены в снимке ниже. Пауза сохраняет этап и шаг. Анализ LLM не означает запуск кода.', ''] : []),
...
export function downloadReport(title, runs, conversation, workspace) {
  ...
  link.href = url
  link.download = memory ? (workspace?.workflow?.blocked_events ? 'day-15-lifecycle.md' : workspace?.invariants ? 'day-14-invariants.md' : workspace?.workflow ? 'day-13-task-workflow.md' : workspace?.profile?.preferences ? 'day-12-personalization.md' : 'day-11-memory-layers.md') : 'day-10-context-strategies.md'
  link.click()
