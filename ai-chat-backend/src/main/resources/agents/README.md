...
`day5-model-comparison.yml` добавляет список `feature_config.models`. Каждая
запись содержит провайдера, API-модель, официальные ссылки и тариф. Ключ Mistral
передаётся отдельно через окружение и никогда не записывается в профиль.
