...
public class DeepSeekClient implements LlmClient {
    ...
    public LlmResult complete(
        ...
        var config = profile.deepseek();
        var request = new LlmRequest(
                StringUtils.hasText(config.model()) ? config.model() : properties.model(),
                messages,
                config.thinking() == null ? null : new Thinking(config.thinking()),
                config.reasoningEffort(),
                config.temperature(),
                config.topP(),
                responseMode.maxTokens(),
                responseMode.stop(),
                responseMode.responseFormat() == null
                        ? null
                        : new ResponseFormat(responseMode.responseFormat())
        );
