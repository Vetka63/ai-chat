...
from application.api_models import CreateBranches, CreateCheckpoint, CreateConversation, SelectModel
...
def create_app(
    ...
    async def lifespan(app: FastAPI):
        ...
        app.state.facts = SqliteFactsRepository(active_store)
    ...
    async def configure_context(agent_id: str, conversation_id: str, body: ContextSettings, request: Request):
        request.app.state.registry.get(agent_id)
        await request.app.state.store.get(agent_id, conversation_id)
        raise AgentError(
            "context_settings_immutable",
            "Стратегия контекста выбирается при создании чата и после этого не изменяется",
            409,
        )

    @app.get("/api/v1/agents/{agent_id}/conversations/{conversation_id}/checkpoints")
    async def checkpoints(agent_id: str, conversation_id: str, request: Request):
        agent = request.app.state.registry.get(agent_id)
        method = getattr(agent, "list_checkpoints", None)
        if method is None:
            raise AgentError("branching_not_supported", "Агент не поддерживает ветвление", 501)
        return await method(conversation_id)

    @app.post("/api/v1/agents/{agent_id}/conversations/{conversation_id}/checkpoints", status_code=201)
    async def create_checkpoint(agent_id: str, conversation_id: str, body: CreateCheckpoint, request: Request):
        agent = request.app.state.registry.get(agent_id)
        method = getattr(agent, "create_checkpoint", None)
        if method is None:
            raise AgentError("branching_not_supported", "Агент не поддерживает ветвление", 501)
        return await method(conversation_id, body.title)

    @app.post("/api/v1/agents/{agent_id}/checkpoints/{checkpoint_id}/branches", response_model=list[ConversationSummary], status_code=201)
    async def create_branches(agent_id: str, checkpoint_id: str, body: CreateBranches, request: Request):
        agent = request.app.state.registry.get(agent_id)
        method = getattr(agent, "create_branches", None)
        if method is None:
            raise AgentError("branching_not_supported", "Агент не поддерживает ветвление", 501)
        return await method(checkpoint_id, body.names)
