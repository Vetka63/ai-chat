...
- разрешает сделать ответ кандидатом только для текущей фазы;
...
11. Проверить output по lifecycle и invariants.
12. Если запрос относится к текущей фазе, сохранить ответ как candidate.
13. При установке candidate увеличить workflow revision.
```
