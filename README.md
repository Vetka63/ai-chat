...
- **day4-temperature** — сравнение одной задачи при температурах `0`, `0.7` и `1.2`.
...
Пространство **«День 4 · Температура»** отправляет одну и ту же задачу тремя
...
UI показывает три ответа рядом. Пользователь оценивает точность и креативность
каждого ответа по шкале 1–5, отдельно оценивает разнообразие, выбирает лучший
вариант, пишет вывод и копирует готовый Markdown-отчёт. История не передаётся и
не хранится. Ошибка одного запроса не скрывает два успешных результата.
